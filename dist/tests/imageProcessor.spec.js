...
describe('ImageProcessor', () => {
    ...
    beforeAll(async () => {
        ...
        await promises_1.default.mkdir(testThumbDir, { recursive: true });
        ...
        imageProcessor = new imageProcessor_1.ImageProcessor(testFullSizeDir, testThumbDir);
    });
    afterAll(async () => {
        try {
            await promises_1.default.rm(testFullSizeDir, { recursive: true, force: true });
            await promises_1.default.rm(testThumbDir, { recursive: true, force: true });
        }
        catch (error) {
    ...
    describe('processImage', () => {
        it('should successfully process an image with valid parameters', async () => {
            ...
            const stats = await promises_1.default.stat(result);
    ...
    describe('listAvailableImages', () => {
        ...
        it('should filter only image files', async () => {
            await promises_1.default.writeFile(path_1.default.join(testFullSizeDir, 'test.txt'), 'This is not an image');
            const images = await imageProcessor.listAvailableImages();
            expect(images).toContain(testImageName);
